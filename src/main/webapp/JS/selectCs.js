...
function selectAllAc() {
    // 获取全选复选框的状态
    var isChecked = document.getElementById("select_all_ac").checked;

    // 获取所有行的复选框
    var checkboxes = document.getElementsByName("row-checkbox-ac");
//显示隐藏的btn
    var btn = document.getElementById("deleteCheckedAc");
    if(isChecked){
        btn.style.display = "block";
    }else
    {
...
function showAcBtn(){
    ...
    if (showbtn) {
        ...
        btn.style.display = "none";
        //取消全选
        document.getElementById("select_all_ac").checked = false;
    }
}
...
function DeleteAllAc() {
    ...
    if(confirm("确定要删除选中的市场活动的记录吗？")){
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "deleteCheckedAcServlet", true); // 替换为后端接口的URL
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
                // 处理 Servlet 返回的数据
