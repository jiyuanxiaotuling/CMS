...
//    查询用户信息（无参数版）
    ...
    public ArrayList findAcInfo(){
        try{
            pstmt=ct.prepareStatement("select * from activity");
            ArrayList al = new ArrayList();
            ResultSet rs=pstmt.executeQuery();
            while(rs.next()){
                AC ac_info = new AC();
                ac_info.setAc_id(rs.getString(1));
                ac_info.setAc_name(rs.getString(2));
                ac_info.setAc_kind(rs.getString(3));
                ac_info.setAc_content(rs.getString(4));
                ac_info.setAc_address(rs.getString(5));
                ac_info.setAc_btime(rs.getDate(6));
                ac_info.setAc_etime(rs.getDate(7));
                al.add(ac_info);
            }
            return al;
        }catch(Exception e){
    ...
    public ArrayList findPtInfo(){
        try{
            pstmt=ct.prepareStatement("select * from participat");
            ArrayList al=new ArrayList();
            ResultSet rs=pstmt.executeQuery();
            while(rs.next()){
                PT pt_info=new PT();
                pt_info.setAc_id(rs.getString(1));
                pt_info.setAc_id(rs.getString(1));
                al.add(pt_info);
            }
            return al;
    ...
    public ArrayList findCsInfoByAc_id(String id){
        try{
            pstmt=ct.prepareStatement("select * from participat where ac_id=?");
            ...
            while(rs.next()){
                PT pt_info=new PT();
                pt_info.setAc_id(rs.getString(1));
                pt_info.setCs_id(rs.getString(2));
                al.add(pt_info);
            }
            return al;
        }catch(Exception e){
...
//    根据分类和关键字查询用户信息
    ...
    public ArrayList findAcInfo(String br,String key){
        try{
            if(key == null){
                pstmt = ct.prepareStatement("select * from activity");
            }else{
                pstmt=ct.prepareStatement("select * from activity where "+ br +" like ?");
                pstmt.setString(1,"%" + key + "%");
            }
            ArrayList al = new ArrayList();
            ResultSet rs=pstmt.executeQuery();
    ...
    public ArrayList findPtInfo(String br,String key){
        try{
            if(key == null){
                pstmt = ct.prepareStatement("select * from participat");
            }else{
                pstmt=ct.prepareStatement("select * from participat where "+ br +" like ?");
                pstmt.setString(1,"%" + key + "%");
            }
            ArrayList pt_al = new ArrayList();
            ResultSet rs=pstmt.executeQuery();
            while(rs.next()){
                PT pt_info=new PT();
                pt_info.setAc_id(rs.getString(1));
    ...
    public ArrayList findEmfoAc(String key){
        try{

                pstmt=ct.prepareStatement("select * from employee where ac_id=?");
                pstmt.setString(1,key);
            ArrayList em_al = new ArrayList();
            ResultSet rs=pstmt.executeQuery();
            while(rs.next()){
                ...
                em_info.setEm_phone(rs.getString(7));
                em_info.setEm_email(rs.getString(8));
                em_info.setAc_id(rs.getString(9));
                em_info.setEm_addtime(rs.getDate(10));
                em_al.add(em_info);
            }
            return em_al;
...
//   添加用户
    ...
    public boolean addPt(PT pt){
        try{
            pstmt=ct.prepareStatement("insert into participat (ac_id,cs_id) values(?,?)");
            pstmt.setString(1, pt.getAc_id());
            pstmt.setString(2, pt.getCs_id());
            pstmt.executeUpdate();
            return true;
        }catch(Exception e){
            e.printStackTrace();
    ...
    public boolean addAc(AC ac){
        try{
            pstmt=ct.prepareStatement("insert into activity (ac_id,ac_name,ac_kind,ac_content,ac_address,ac_btime,ac_etime) values(?,?,?,?,?,?,?)");
            pstmt.setString(1, ac.getAc_id());
            pstmt.setString(2, ac.getAc_name());
            pstmt.setString(3, ac.getAc_kind());
            pstmt.setString(4, ac.getAc_content());
            pstmt.setString(5, ac.getAc_address());
            pstmt.setDate(6,  (Date) ac.getAc_btime());
            pstmt.setDate(7,  (Date) ac.getAc_etime());
            pstmt.executeUpdate();
            return true;
        }catch(Exception e){
            e.printStackTrace();
...
//    修改用户信息
    ...
    //    修改用户信息
    public boolean modifyAc(AC ac){
        try{
            pstmt=ct.prepareStatement("UPDATE activity SET ac_name=?,ac_kind=?,ac_content=?,ac_address=?,ac_btime=?,ac_etime=? WHERE ac_id=?");
            pstmt.setString(1, ac.getAc_name());
            pstmt.setString(2, ac.getAc_kind());
            pstmt.setString(3, ac.getAc_content());
            pstmt.setString(4, ac.getAc_address());
            pstmt.setDate(5, (Date)ac.getAc_btime());
            pstmt.setDate(6, (Date)ac.getAc_etime());
            pstmt.setString(7, ac.getAc_id());
            pstmt.executeUpdate();
            return true;
        }catch(Exception e){
            e.printStackTrace();
    ...
    public boolean modifyEm(EM em){
        try{
            pstmt=ct.prepareStatement("UPDATE employee SET em_name=?,em_gender=?,em_age=?,em_position=?,em_department=?,em_phone=?,em_email=?,ac_id=? WHERE em_id=?");
            ...
            pstmt.setString(8, em.getAc_id());
            pstmt.setString(9, em.getEm_id());
            pstmt.executeUpdate();
            return true;
        }catch(Exception e){
            e.printStackTrace();
...
//    删除用户
    ...
    public boolean deletePt(String ac_id,String cs_id){
        try{
            pstmt=ct.prepareStatement("DELETE from participat WHERE ac_id=? AND cs_id=?");
    ...
    public boolean deleteAc(String id){
        try{
            pstmt=ct.prepareStatement("DELETE from activity WHERE ac_id=?");
            pstmt.setString(1,id);
            pstmt.executeUpdate();
            return true;
        }catch(Exception e){
